...
!*****************************************************************************************
!
!  subroutine: csolve (written by Chris Sims in Matlab, translated by Huixin Bi)
!
!*****************************************************************************************
...
subroutine csolve(x_csolve,index_csolve, nx_csolve, tol_csolve,nmax_csolve,rc_csolve,nstate_csolve)
    ...
    real(dp), parameter :: delta_csolve = 0.000001_dp !(0.1_dp)**6       ! differencing interval for numerical gradient
    real(dp), parameter :: alpha_csolve = 0.001_dp !(0.1_dp)**3       ! tolerance on rate of descent 
    ...
    ! initialization
    f0_csolve = fcn_eulor(x_csolve,index_csolve)    
    ...
    do while (done_csolve==0)
        ...
        if ((itct_csolve>3) .AND. (af00_csolve-af0_csolve<tol_csolve*max(1.0_dp,af0_csolve)) .AND. (itctmod_csolve==1)) then
            ...
            do ix_csolve=1,nx_csolve
                ...
                grad_csolve(:,ix_csolve)=(fcn_eulor(xj_csolve,index_csolve)-f0_csolve(:))/h_csolve(ix_csolve)                
        ...
        if (randomize_csolve == 1) then                
            ...
            method=VSL_METHOD_SGAUSSIAN_ICDF 
            seed= 10100             
        ...
        do while (subdone_csolve==0)
            ...
            f_csolve = fcn_eulor(x_csolve+dx_csolve,index_csolve)
